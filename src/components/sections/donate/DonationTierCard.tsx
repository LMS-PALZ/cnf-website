...
import {
  type DonationTier,
  type DonationTierVariant,
} from "@/data/donate/donation-tiers";
...
type Variants = Record<
  DonationTierVariant,
  {
    /** Top section: amount band. */
    headerBg: string;
    headerAmount: string;
    headerSubtitle: string;
    /** Body / overall card surface. */
    cardBorder: string;
    /** Submit button variant on the bottom. */
...
const variants: Variants = {
  "soft-green": {
    ...
    buttonClassName: "bg-cnf-primary text-white hover:bg-cnf-primary-dark",
  },
  "soft-gold": {
    ...
    buttonClassName:
      "bg-cnf-accent text-cnf-accent-ink hover:bg-cnf-accent-hover",
  },
  "solid-green": {
  ...
  "solid-dark": {
    ...
    cardBorder: "border-cnf-border",
    buttonClassName:
      "bg-cnf-night text-white hover:bg-cnf-night-soft border border-cnf-night-border",
  },
};
...
/**
 * Single donation tier card. The card has two visual sections:
 * - a coloured top band with the amount + subtitle (and an optional badge)
 * - a white body with title, description, bullets, and the CTA button
 */
export function DonationTierCard({ tier }: Props) {
  ...
  return (
    ...
    >
      {tier.badge ? (
        <span className="absolute right-4 top-4 z-10 inline-flex items-center rounded-full bg-cnf-accent px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.2em] text-cnf-accent-ink">
          {tier.badge}
        </span>
      ) : null}
      ...
      <div className={cn("px-6 pb-5 pt-7", v.headerBg)}>
        <p
          className={cn(
            ...
            "text-3xl font-semibold leading-none md:text-4xl",
            v.headerAmount
          )}
        >
          {tier.amount}
      ...
      <div className="flex flex-1 flex-col p-6">
        <h3
          className={cn(
            fontDisplay.className,
            "text-lg font-semibold text-cnf-ink"
          )}
        >
          {tier.title}
        </h3>
        ...
        <div className="mt-6 flex flex-1 flex-col justify-end">
          <ButtonLink
            ...
            variant="primary"
            size="md"
            className={cn("w-full", v.buttonClassName)}
          >
            {tier.ctaLabel}
          </ButtonLink>
        </div>
