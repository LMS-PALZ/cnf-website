...
type Props = {
  ...
  /** Optional override — drop in a real `<Image>` later. */
  media?: React.ReactNode;
};

/**
 * Right-hand column inside a `PillarBlock`: a coloured image card with a
 * dark "headline reach stat" overlaying the bottom, plus a 2×2 grid of
 * supporting stats below.
 */
export function PillarMediaCard({
  theme,
  mediaLabel,
  reachStat,
  smallStats,
          ...
          <Card tone="dark" padding="md" className="bg-cnf-night/95 backdrop-blur">
            ...
            <p
              className={cn(
                ...
                "mt-2 text-4xl font-semibold leading-none text-white md:text-5xl"
              )}
            >
              {reachStat.value}
            </p>
