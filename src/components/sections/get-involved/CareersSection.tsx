...
/**
 * Careers section: "Build Your Career With Purpose" + 2x2 Life-at-CNF
 * tiles on the left, newsletter signup and profile-submission forms on
 * the right.
 */
        ...
        <div className="grid gap-12 lg:grid-cols-2 lg:items-start">
          ...
          <div>
            <SectionHeader
              ...
              accent="primary"
              description="Working at CNF means your skills do something that matters. We are a small, dedicated team committed to doing excellent work for communities that deserve nothing less. If that resonates with you, we want to hear from you."
              id="careers-title"
            />
            ...
            <ul className="mt-4 grid gap-4 sm:grid-cols-2">
              {lifeAtCnfCards.map((card) => {
                ...
                return (
                  <li key={card.id}>
                    <Card
                      tone={solid ? "primary" : "outlined"}
                      padding="md"
                      className={cn(
                        "h-full",
                        !solid && "border-cnf-border"
                      )}
                    >
                      <span
                        aria-hidden
                        className={cn(
                          "inline-flex h-8 w-8 items-center justify-center rounded-md [&_svg]:h-4 [&_svg]:w-4",
                          solid
                      ...
                      <p
                        className={cn(
                          ...
                          solid ? "text-white/85" : "text-cnf-muted"
                        )}
                      >
                        {card.description}
                      </p>
          ...
          {/* Right — newsletter + profile forms */}
          <div className="space-y-6">
            <div className={`${formCardSurface} text-center`}>
              <span
                aria-hidden
                className="inline-flex h-10 w-10 items-center justify-center rounded-md bg-cnf-accent/15 text-cnf-accent-hover [&_svg]:h-5 [&_svg]:w-5"
              >
                <ClipboardIcon />
              </span>
              <h3
                className={`${fontDisplay.className} mt-4 text-xl font-semibold text-cnf-ink md:text-2xl`}
              >
            ...
            <div className={formCardSurface}>
              <h3
                className={`${fontDisplay.className} text-xl font-semibold text-cnf-ink md:text-2xl`}
              >
