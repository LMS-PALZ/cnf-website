...
export function CareersNewsletterForm() {
    const id = useId();
    const { register, handleSubmit, setError, reset, formState: { errors, isSubmitting }, } = useForm<CareersNewsletterInput>({
    ...
    return (<form noValidate onSubmit={onSubmit} className="space-y-2">
      <div className="flex flex-col gap-2 sm:flex-row">
        <div className="flex-1">
          <FormField id={`${id}-email`} label="Email" hideLabel error={errors.email?.message}>
            <input id={`${id}-email`} type="email" autoComplete="email" placeholder="yourname@email.com" aria-invalid={errors.email ? true : undefined} className={inputCompact} {...register("email")}/>
          </FormField>
        </div>
