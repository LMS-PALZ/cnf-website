...
export function VolunteerForm() {
    ...
    const { register, handleSubmit, setError, reset, formState: { errors, isSubmitting }, } = useForm<VolunteerInput>({
    ...
    return (<form className="space-y-5" noValidate onSubmit={onSubmit}>
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        <FormInput id={`${idPrefix}-city`} label="City" required placeholder="Your city" autoComplete="address-level2" error={errors.city?.message} {...register("city")}/>
      ...
      <FormSelect id={`${idPrefix}-area`} label="Which area would you like to volunteer in?" required placeholder="Select an area" options={volunteerAreaOptions} error={errors.volunteerArea?.message} {...register("volunteerArea")}/>

      <FormSelect id={`${idPrefix}-prior`} label="Do you have prior volunteering experience?" required placeholder="Select" options={yesNoOptions} error={errors.priorExperience?.message} {...register("priorExperience")}/>

      <FormInput id={`${idPrefix}-priorOrg`} label="If yes, which organisation?" placeholder="Organisation name" error={errors.priorOrganisation?.message} {...register("priorOrganisation")}/>

      <FormSelect id={`${idPrefix}-leadership`} label="Are you open to teaching or leadership roles?" required placeholder="Select" options={leadershipOptions} error={errors.leadershipOpenness?.message} {...register("leadershipOpenness")}/>

      <FormSelect id={`${idPrefix}-idType`} label="Type of ID" required placeholder="Select ID type" options={idTypeOptions} error={errors.idType?.message} {...register("idType")}/>
