...
/**
 * Filter tabs shown above the project grid. `all` is a meta-filter that
 * matches every project regardless of pillar.
 */
export type ProjectFilter = "all" | "skills" | "education" | "humanitarian";

/** Visual theme used by `ProjectCard`. Mirrors `PillarTheme` plus the
 * derived `healthcare` tag (renders identical to `humanitarian`). */
export type ProjectTheme = "skills" | "education" | "humanitarian";

export type ProjectItem = {
  id: string;
  /** Pillar this project rolls up to — used for filtering. */
  pillar: Exclude<ProjectFilter, "all">;
  /** Visual theme — usually mirrors `pillar`. */
  theme: ProjectTheme;
  /** Badge label inside the card (e.g. "Healthcare", "Skill Development"). */
  badge: string;
  title: string;
  description: string;
  /** Photo placeholder copy displayed in the card's coloured band. */
  mediaLabel: string;
  /** Icon shown centred above the photo placeholder. */
  iconKey: IconRegistryKey;
  /** Optional link — if omitted the "Read more" link points to `#`. */
  href?: string;
};

export const filterTabs: { id: ProjectFilter; label: string }[] = [
...
export const projects: ProjectItem[] = [
  ...
  },
  {
    ...
    description:
      "Essential food items distributed to vulnerable families in Mangu, Plateau State — part of CNF's ongoing commitment to communities across Nigeria.",
  ...
  },
  {
    ...
    description:
      "Free medical services, scholarships, and food relief delivered to displaced families at New Kuchingoro IDP Camp — meeting people in one of their most vulnerable moments.",
